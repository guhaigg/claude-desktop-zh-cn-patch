...
补丁覆盖语言 JSON 文件，并修补少量仍然硬编码英文的前端资源。不修改账号、API key、会话数据或 Claude Code 配置。
...
- `ion-dist/assets/v1/*.js`：修补语言表无法覆盖的硬编码界面文字。
...
1. Claude 更新后可能覆盖语言文件和前端资源，需要重新运行 `install.ps1`。
